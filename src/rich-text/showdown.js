	...
	this._dispatch = function dispatch (evtName, text, options, globals) {
		if (listeners.hasOwnProperty(evtName)) {
			for (let ei = 0; ei < listeners[evtName].length; ++ei) {
				let nText = listeners[evtName][ei](evtName, text, this, options, globals);
   ...
   */
	this.removeExtension = function (extension) {
		...
		for (let a = 0; a < extension.length; ++a) {
			let ext = extension[a];
			for (let i = 0; i < langExtensions.length; ++i) {
...
showdown.subParser('anchors', function (text, options, globals) {
	...
	let writeAnchorTag = function (wholeMatch, m1, m2, m3, m4, m5, m6, m7) {
...
showdown.subParser('autoLinks', function (text, options, globals) {
	...
	let simpleURLRegex  = /\b(((https?|ftp|dict):\/\/|www\.)[^'">\s]+\.[^'">\s]+)(?=\s|$)(?!["<>])/gi,
	...
	function replaceMail(wholeMatch, m1) {
		let unescapedStr = showdown.subParser('unescapeSpecialChars')(m1);
...
showdown.subParser('blockGamut', function (text, options, globals) {
	...
	// Do Horizontal Rules:
	let key = showdown.subParser('hashBlock')('<hr />', options, globals);
...
showdown.subParser('githubCodeBlocks', function (text, options, globals) {
	...
	text = text.replace(/(?:^|\n)```(.*)\n([\s\S]*?)\n```/g, function (wholeMatch, language, codeblock) {
		...
		// store the primitive text and the parsed text in a global let,
...
showdown.subParser('hashHTMLSpans', function (text, config, globals) {
	...
	let matches = showdown.helper.matchRecursiveRegExp(text, '<code\\b[^>]*>', '</code>', 'gi');
...
showdown.subParser('hashPreCodeTags', function (text, config, globals) {
	...
	let repFunc = function (wholeMatch, match, left, right) {
		...
		let codeblock = left + showdown.subParser('encodeCode')(match) + right;
		return '\n\n~G' + (globals.ghCodeBlocks.push({ text: wholeMatch, codeblock: codeblock }) - 1) + 'G\n\n';
	};
...
showdown.subParser('headers', function (text, options, globals) {
	...
	text = text.replace(/^(#{1,6})[ \t]*(.+?)[ \t]*#*\n+/gm, function (wholeMatch, m1, m2) {
		let span = showdown.subParser('spanGamut')(m2, options, globals),
	...
	function headerId(m) {
		let title, escapedId = m.replace(/[^\w]/g, '').toLowerCase();
...
showdown.subParser('tables', function (text, options, globals) {
	...
	function parseCells(cell, style) {
		let subText = showdown.subParser('spanGamut')(cell, options, globals);
